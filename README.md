...
## File Tree
The main upper-level files are in the rmcs_core function package

hardware：机器人上硬件控制的封装，以及FSM的实现

controller：控制逻辑的部分，臂和底盘互不干涉，切换不同模式时，底盘会根据臂的状态有个预设。

engineer.cpp：关于硬件接口的分配以及硬件发送的实现
